...
Download_adbyby(){
	...
	cd bin
	chmod 777 adbyby
	wget -N --no-check-certificate "https://raw.githubusercontent.com/ojbk8/doubi/master/other/adhook.ini"
}
Service_adbyby(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ojbk8/doubi/master/service/adbyby_centos -O /etc/init.d/adbyby; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ojbk8/doubi/master/service/adbyby_debian -O /etc/init.d/adbyby; then
			echo -e "${Error} ADbyby服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/adbyby
